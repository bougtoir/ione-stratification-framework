...
AI_DATES = 'August 2025 through the date of submission'
...
Figure 4 and Supplementary Table S2 report n = 500, 2000 and 10 000 (K=5). These estimates are from a separate 30-replication sensitivity batch, so point estimates are noisier than the primary scenario. Crude bias declined with n. The residual method's absolute random-effects ATE bias plateaued near 0.008–0.009 across the three sample sizes ({v['n500_re_bias']} at n=500, {v['n2000_sens_re_bias']} at n=2000 and {v['n10000_re_bias']} at n=10 000); the slight differences between this n=2000 value and the 50-replication primary value in Table 1 ({v['re_bias']}) reflect Monte Carlo variation. Its relative reduction therefore fell as the crude marginal estimate became more precise. Propensity-score, prognostic-score and clustering-based methods achieved larger relative reductions at n=10 000 (range {v['n10000_nonresid_rel_min']}–{v['n10000_nonresid_rel_max']}), showing that performance depends on the method as well as sample size. This pattern indicates that stratification can reduce the component of aggregation bias that is predictable from measured covariates, but it cannot fully adjust for unmodelled hidden effect modification.
...
At n = 10 000 the residual method still showed a non-negligible random-effects ATE bias ({v['n10000_re_bias']}; relative reduction {v['n10000_rel_re']}), similar to the absolute floor observed at n=500 ({v['n500_re_bias']}) and n=2000 ({v['n2000_sens_re_bias']}). This floor is consistent with the bounded sorting accuracy reflected in the modest ARI values: once finite-sample error is removed, remaining bias reflects the structural mismatch between the discovered strata and the true CATE surface. The true-CATE-quantile oracle provides an upper bound on what perfect sorting could achieve; the gap between the oracle and the leading data-driven method quantifies the cost of not observing the true effect modifier.
