...
def _sens_df(df, methods, conditions):
    """Build a DataFrame for sensitivity tables."""
    rows = []
...
def main():
    ...
    if not sample_df.empty:
        _add_table(
            doc, sample_df,
            ['Condition', 'Method', 'ARI', 'C1', 'W_est', 'RE bias', 'Relative reduction RE', 'RE I2'],
            'Table 4. Sample-size sensitivity (K=5, z=1.0, zx=1.0): means over 10 simulations.',
            {
                'Condition': 'condition', 'Method': 'method', 'ARI': 'ARI_mean',
                'C1': 'C1_heterogeneity_mean', 'W_est': 'W_est_mean',
                'RE bias': 'abs_bias_re_mean', 'Relative reduction RE': 'bias_reduction_relative_re_mean',
                'RE I2': 're_I2_mean'
            },
            decimal_overrides={'RE bias': 5, 'Relative reduction RE': 3}
    ...
    if nonlinearity is not None and not nonlinearity.empty:
        ...
        if rows:
            ...
            _add_table(
                ...
                decimal_overrides={'Linear RE bias': 5, 'Non-linear RE bias': 5,
                                   'Linear rel reduction': 3, 'Non-linear rel reduction': 3}
            )
