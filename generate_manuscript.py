...
def generate_manuscript():
    ...
    abstract_text = (
        ...
        "We propose two exploratory coherence diagnostics—C1 (between-stratum heterogeneity) and W (within-stratum CATE homogeneity)—and benchmark stratification-based extraction in a simulation with explicit confounding and effect modification. "
        "Methods. We generated binary treatment and outcome data with measured covariates carrying traces of an unmeasured confounder/modifier, and compared four proposed IONE methods, six PCA variants, three active comparators and two oracle baselines. Outcome-informed methods used a 50/50 discovery/evaluation split. The estimand was the population risk-difference ATE; performance was measured by ARI, C1, W and ATE bias reduction. "
        f"Results. In the primary scenario (n=2000, K=5), extraction was modest (best ARI = {best_ari}). Bias reduction was also modest: the best absolute reduction was {best_bias} (relative {best_rel}), from a crude absolute bias of {crude_abs} to a stratified absolute bias of {strat_abs}, achieved by {best_method}. Sensitivity analyses showed that stronger Z→X traces and larger sample sizes improved diagnostics and bias reduction; non-linear Z→X mappings degraded PCA but not clustering-based methods. Semi-synthetic illustrations showed high ARI only when pseudo-variables strongly correlated with the known confounder. "
        "Conclusions. IONE provides transparent, exploratory diagnostics for hidden population structure, but extraction remains conditional and modest. It should be used as a sensitivity complement to established causal-inference methods."
    )
