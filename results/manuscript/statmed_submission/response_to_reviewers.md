...
> This response accompanies the revised manuscript for submission to *Statistics in Medicine* (with *Statistical Methods in Medical Research* as an alternative target). The manuscript has been reframed as an **exploratory diagnostic-sensitivity study** rather than a mature two-stage workflow. All numerical results in the revised manuscript will be regenerated from an updated, version-locked simulation pipeline; the revisions described below reflect the final analytical plan.
...
**Response.** We have reconciled the definition and all reported values. Bias reduction is now reported as the **absolute reduction in log-OR bias** (in percentage points) and the formula in the Methods has been rewritten accordingly:
...
Both values are reported where useful. For Method 1B in the primary simulation the absolute reduction is 0.074 (7.4 percentage points on the log-OR scale); the corresponding relative reduction is approximately 85 %. All tables, figure captions and prose have been edited for internal consistency.
...
**Response.** Same as R1.4. We use **absolute log-OR bias reduction** as the primary reported quantity, with the formula unambiguously stated. Relative reduction is provided as a secondary metric.
...
**Response.** We have implemented a **discovery/evaluation split** for all outcome-informed methods: stratum assignment is learned on a 50% discovery sample and treatment-effect estimation (log-OR, C1, W_est) is performed on the independent 50% evaluation sample. A new sensitivity analysis reports the **induced bias** from using the same data for both assignment and estimation, showing that sample splitting materially reduces this bias. The Discussion states that residual-based stratification is **exploratory only** and should not be used for confirmatory effect estimation without an independent evaluation set.
...
**Response.** We have created a single, exact method inventory used throughout:
...
| Proposed IONE methods (Family 2: outcome-free) | 2A PCA variants (6 configurations), 2B k-means on X |
| Active comparators | PS quintiles, Gaussian mixture model, prognostic score, **k-means on X is also listed as a comparator when it is not framed as an IONE method** |
| Oracle baselines | Oracle k-means on Z, oracle quantile on Z1 |
| Lower baseline | Random stratification |

The Abstract now consistently reports **four active comparators** (PS quintiles, GMM, prognostic score, k-means on covariates) when the PCA variants are aggregated. The Methods enumerate the full list without contradiction, and k-means on X is treated as **both a proposed method (2B) and a comparator baseline** depending on whether it is presented within the IONE framework or as an off-the-shelf clustering approach.
...
**Response.** The revised Methods include an **Estimands** subsection that defines four estimands: (1) subgroup-specific treatment effects τ(z) = E[Y(1)−Y(0) | Z = z]; (2) ATE bias reduction from stratification-adjusted vs. crude estimation; (3) C1 (between-stratum heterogeneity of stratum-specific log ORs); (4) W (within-stratum homogeneity of CATEs). IONE is positioned as an exploratory diagnostic/preprocessing step that identifies populations that may warrant subgroup-specific analysis, not as a complete causal inference method.
...
3. **Method inventory** — four active comparators (PS-Q, GMM, prognostic score, k-means on X) plus proposed and oracle baselines; taxonomy is now consistent across Abstract, Methods, tables and response.
4. **Metrics** — redefined C1 as an exploratory, uncalibrated heterogeneity indicator; added true and estimated W; reconciled bias-reduction formula and values; added calibration curves.
