...
A simulation study of stratification-based diagnostics for hidden effect modification in individual participant data (IPD) meta-analysis.

## Overview

IONE proposes two exploratory diagnostics for treatment-effect estimates in IPD meta-analysis:
- **C1** (within-stratum coherence): 1 − I² from stratum-specific log odds ratios of treatment on outcome.
- **W** (between-stratum CATE variance explained): variance ratio of estimated or true conditional average treatment effects across strata.

The repository implements the data-generating mechanism, stratification methods, evaluation metrics and manuscript-generation pipeline used in the current submission to the *Journal of Computational Mathematics and Data Science* (JCMDS).
...
├── generate_ione_rsm_v3.py     # Generate full JCMDS Word manuscript, title page, cover letter and figures
├── generate_full_rsm_manuscript.py # Generate manuscript figures and widescreen figures .pptx
...
└── results/
    ...
    └── manuscript/jcmds_submission/ # JCMDS submission package (docx, pptx, zip)
```
...
To build the full JCMDS submission package from the same committed CSVs:
...
python3 generate_ione_rsm_v3.py                  # full JCMDS submission package
```
...
- **Primary:** *Journal of Computational Mathematics and Data Science* (Elsevier)

## Key methods

- **Proposed IONE**: 1A predicted probability, 1B residual; 2A PCA, 2B k-means on X.
- **Active comparators**: propensity-score quintiles, Gaussian mixture model, prognostic-score stratification.
- **Oracle baselines**: oracle k-means on Z, oracle quantile on Z1; random stratification as a chance reference.
